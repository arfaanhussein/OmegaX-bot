# -*- coding: utf-8 -*-
"""
OmegaX Fusion v25 (Render-friendly, no Docker)
- Python runtime (3.11 recommended for fast wheels on Render)
- Exchange: Binance USDT-M Futures via ccxt REST; optional python-binance user WS for fills
- Strategies:
  - Ensemble alpha: trend, mean reversion, VWAP, Donchian, pair residual, cross-sectional momentum, carry/funding
  - Scalping (1m)
...
- Persistence: local JSON + optional GitHub Gist remote (no S3 to keep light)
...
PERSIST_MODE = os.environ.get("PERSIST_MODE", "").lower()  # local|remote|both
    ...
    if mode == "both" and remote:
        class Both(BasePersistence):
            def __init__(self, l, r):
                self.l, self.r = l, r
            def load(self):
                return self.r.load() or self.l.load()
            def save(self, *args, **kwargs):
                self.l.save(*args, **kwargs)
                try:
                    self.r.save(*args, **kwargs)
                except Exception as e:
                    logging.warning(f"Remote save failed: {e}")
...
class ScalpingStrategy:
    ...
    def analyze(self, symbol: str, df1m: pd.DataFrame) -> Optional[dict]:
        if df1m is None or df1m.empty or len(df1m) < max(50, SCALPING_BB_PERIOD + 2):
            return None
        closes = df1m["close"].values
        highs = df1m["high"].values
        lows = df1m["low"].values
        volumes = df1m["volume"].values
        rsi = RSIIndicator(df1m["close"], SCALPING_RSI_PERIOD).rsi().iloc[-1]
        bb = BollingerBands(df1m["close"], SCALPING_BB_PERIOD, 2)
        bb_u = bb.bollinger_hband().iloc[-1]
        bb_l = bb.bollinger_lband().iloc[-1]
        price = float(closes[-1])
        bb_pos = (price - bb_l) / max(1e-9, (bb_u - bb_l))
        vol_ma = float(pd.Series(volumes).rolling(20).mean().iloc[-1] or 0)
        volume_spike = (volumes[-1] / vol_ma) if vol_ma > 0 else 1.0
...
class MeanReversionStrategy:
    ...
    def analyze(self, symbol: str, df: pd.DataFrame) -> Optional[dict]:
        if df is None or df.empty or len(df) < max(50, ZSCORE_PERIOD + 2):
            return None
        closes = df["close"].values.astype(float)
        series = pd.Series(closes)
        roll_mean = series.rolling(ZSCORE_PERIOD).mean()
        roll_std = series.rolling(ZSCORE_PERIOD).std()
        z = (series - roll_mean) / (roll_std + 1e-9)
        zlast = float(z.iloc[-1]) if not np.isnan(z.iloc[-1]) else 0.0
        price = float(closes[-1])
        mean_price = float(np.mean(closes[-ZSCORE_PERIOD:])) if len(closes) >= ZSCORE_PERIOD else price
        if zlast < -ZSCORE_THRESHOLD:
            return {"side": "long", "entry": price, "sl": price * 0.98, "tp": mean_price, "confidence": float(min(abs(zlast) / 3.0, 1.0))}
        if zlast > ZSCORE_THRESHOLD:
            return {"side": "short", "entry": price, "sl": price * 1.02, "tp": mean_price, "confidence": float(min(abs(zlast) / 3.0, 1.0))}
        return None
...
class ExecutionEngine:
    ...
    def replace_brackets(self, symbol: str, positions, new_qty: float, new_sl: float, new_tp: float, risk, notifier, hedge_book=None):
        ...
        if not p or p.mode in ("hedge", "temp_hedge"):
            return
        with self.lock:
            p.sl = new_sl
            p.tp = new_tp
...
class FundingCache:
    ...
    def maybe_update(self):
        ...
        if not self.symbols:
            self.last = now
            return
        s = self.symbols[self._idx % len(self.symbols)]
        self._idx += 1
        try:
    ...
    def evaluate(self, sym: str, df5: pd.DataFrame, price: float):
        with self.bot.lock:
            ...
            if open_dirs >= MAX_CONCURRENT_POS:
                return
            if self.bot.positions.get(sym):
                return
        if df5 is None or df5.empty or len(df5) < 100 or not self._can_fire(sym):
            return
        if self.bot.portfolio_manager.bucket_violation():
            return
        if not self.bot.risk.can_trade(sym):
            return
        highs = df5["high"].tail(288)
        lows = df5["low"].tail(288)
        if len(highs) < 50:
            return
        h, l = float(np.max(highs)), float(np.min(lows))
        mid = (h + l) / 2.0
        span = max(1e-9, (h - l))
        level_gap = max(price * GRID_SPACING_PCT, span / (GRID_LEVELS * 2.0))
        idx = int((price - mid) / level_gap)
        up_level = mid + ((idx + 1) * level_gap)
        dn_level = mid + ((idx - 1) * level_gap)
        tol = level_gap * 0.4
        side = None
        if abs(price - dn_level) <= tol:
            side = "long"
        elif abs(price - up_level) <= tol:
            side = "short"
        else:
            return
        ...
        alpha_boost = self.bot.alpha.boost(sym)
        lev = self.bot.risk.dynamic_leverage(False, alpha_boost)
        base = (RISK_PER_TRADE / max(1, len(self.bot.symbols))) * self.bot.risk.risk_multiplier(False, alpha_boost) * GRID_RISK_FRACTION
        with self.bot.lock:
            open_list = [p for p in self.bot.positions.values() if p]
        ...
        if opened:
            self.bot.risk.note_open(sym)
            self.bot.hedger.open_temp_hedge_for(sym, side, qty, price)
...
class ArbitrageCoordinator:
    def __init__(self, notifier: Notifier):
        ...
        for exid in ARB_EXCHANGES:
            try:
                exclass = getattr(ccxt, exid)
                ex = exclass({"enableRateLimit": True, "timeout": HTTP_TIMEOUT * 1000})
                ex.load_markets()
                self.exchanges[exid] = ex
            except Exception as e:
                logging.warning(f"Arb exchange {exid} init failed: {e}")
        self.last_scan = 0.0
    def scan(self, symbols: List[str]):
        if not ENABLE_ARBITRAGE or not self.exchanges or time.time() - self.last_scan < ARB_SCAN_INTERVAL_SEC:
            return
            ...
            for sym in subset:
                ...
                for i in range(len(quotes)):
                    for j in range(len(quotes)):
                        if i == j:
                            continue
                        ex_buy, _, ask = quotes[i]
                        ex_sell, bid, _ = quotes[j]
                        if ask <= 0:
                            continue
                        gross = (bid - ask) / ask
                        net = gross - 0.002
                        if net > ARB_MIN_NET_PCT:
                            best = (sym, ex_buy, ex_sell, net, bid, ask)
                if best:
...
class OmegaXBot:
    def __init__(self):
        ...
        self.notifier.send_critical(f"🚀 Starting OmegaX Fusion v25 ({MODE}).")
    ...
    def _start_health_server(self):
        ...
        def index():
            return "OmegaX Fusion v25 running. Endpoints: /health, /metrics, /ping, /api/status, /api/positions", 200
    ...
    def _on_signal_event(self, ev: Event):
        ...
        sl = sig.get("sl")
        tp = sig.get("tp")
        if not s or s not in self.symbols or side not in ("long", "short") or not sl or not tp:
            return
        price = self.prices.get(s, self.data.get(s, "5m"))
        if not price:
            return
        with self.lock:
            has_pos = bool(self.positions.get(s))
        if has_pos or self.risk.risk_off or self.portfolio_manager.bucket_violation():
            return
        f_alpha = self.alpha.boost(s)
        lev = self.risk.dynamic_leverage(False, f_alpha)
    ...
    def _manage_position(self, s: str, price: float, df5: pd.DataFrame):
        ...
        if p.time_limit and time.time() >= p.time_limit:
            self.engine.close(s, self.positions, price, "SCALP_TIME_LIMIT", self.risk, self.notifier, hedge_book=self.hedger.snapshot())
            return
        ...
        if r > 0:
            ...
            if r_mult >= TEMP_HEDGE_CLOSE_AT_R:
                self.hedger.close_temp_hedge_for(s)
    ...
    def _periodic_report(self):
        if time.time() - self.last_report < TELEGRAM_REPORT_INTERVAL_SEC:
            return
        with self.lock:
            rb = self.risk.balance
            dd = self.risk.drawdown
    ...
    def run(self):
        try:
                ...
                try:
                    ...
                    for _, s in priorities:
                        ...
                        if has_pos:
                            ...
                            continue
                        if ENABLE_GRID and self.grid:
                            self.grid.evaluate(s, df5, price)
            ...
            try:
                self.notifier.send_critical(f"💥 OmegaX Fusion fatal crash: {str(e)[:200]}")
            except Exception:
                pass
            raise
