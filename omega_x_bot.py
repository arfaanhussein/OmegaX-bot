# -*- coding: utf-8 -*-
"""
OmegaX Fusion v24 (Render-friendly, no Docker)
- Python runtime friendly (3.11 recommended for speed on Render)
- Exchange: Binance USDT-M Futures via ccxt REST; optional python-binance WS for fills
- Strategies:
  - Ensemble alpha: trend, meanreversion, VWAP, Donchian, pair residual, cross-sectional momentum, carry/funding
  - Scalping (1m)
  - Mean Reversion (Z-score)
  - Grid (reactive)
  - Arbitrage (detect-only alerts; disabled by default)
- Risk: per-trade risk, portfolio cap, drawdown pause/halt, cooldowns
- Execution: market entries + native SL/TP brackets, cancel&replace, repair loop, emergency flatten
- Hedging: portfolio overlay hedge in BTC + temp entry hedge
- EventBus: closed-candle events via REST polling (no ccxt-pro)
- Persistence: local JSON + optional GitHub Gist remote (no S3/boto3 to keep light)
- KeepAlive pinger + Flask health endpoints + 15m Telegram reports
"""
...
# Persistence config (lightweight: local JSON + optional Gist)
PERSIST_MODE = os.environ.get("PERSIST_MODE", "").lower()  # local|remote|both (auto below)
REMOTE_BACKEND = os.environ.get("REMOTE_BACKEND", "").lower()  # gist
        ...
        while self.running:
            try:
                if self.external_url:
                    try: requests.get(self.external_url, timeout=8)
                    except Exception: pass
                if self.self_url:
                    try: requests.get(self.self_url, timeout=8)
                    except Exception: pass
            except Exception:
                pass
...
class DataFeed:
    ...
    def schedule_updates(self, max_updates=MAX_KLINE_UPDATES_PER_LOOP) -> int:
        ...
        budget = min(max_updates + int(0.08 * len(self.syms) * len(TIMEFRAMES)), 36)
...
class GistPersistence(BasePersistence):
    ...
    def save(self, risk, positions: Dict[str, Optional["Position"]], pending: List[dict] | None = None, hedge_book: dict | None = None):
        try:
            ...
            body = {"files": {self.filename: {"content": json.dumps(serial, indent=2)}}}
...
def build_persistence() -> BasePersistence:
    ...
    remote = None
    if REMOTE_BACKEND == "gist" and all([GITHUB_TOKEN, GIST_ID]):
    ...
    if mode == "both" and remote:
        class Both(BasePersistence):
            def __init__(self, l, r): self.l, self.r = l, r
            def load(self): return self.r.load() or self.l.load()
            def save(self, *args, **kwargs):
                self.l.save(*args, **kwargs)
                try: self.r.save(*args, **kwargs)
                except Exception as e: logging.warning(f"Remote save failed: {e}")
        return Both(local, remote)
    return local

# ====================== DOMAIN + RISK + META + STRATEGIES ======================
...
class RiskManager:
    ...
    def dynamic_leverage(self, vol_spike: bool, alpha_boost: float) -> float:
        base = 14.0
        if vol_spike: base *= 0.75
        if self.consec_losses >= 2: base *= 0.85
        if self.drawdown > 0.02: base *= 0.9
    ...
    def risk_multiplier(self, vol_spike: bool, alpha_boost: float) -> float:
        mult = 1.0
        if self.consec_losses >= 2: mult *= 0.8
        if vol_spike: mult *= 0.75
        if self.risk_off: mult *= 0.5
...
class SymbolAlpha:
    ...
    def record(self, pnl: float):
        self.pnls.append(pnl); self.pnls = self.pnls[-50:]
    def expectancy(self) -> float:
        if not self.pnls: return 0.0
        wins = [x for x in self.pnls if x >= 0]; losses = [x for x in self.pnls if x < 0]
...
@dataclass
class Welford:
    n: int = 0; mean: float = 0.0; m2: float = 0.0
...
class MetaPolicy:
    ...
    def sample_weights(self, symbol: str) -> Dict[str, float]:
        ...
        vals = np.array(list(out.values())); vals -= np.max(vals)
        ws = np.exp(vals); denom = np.sum(ws); 
        if denom > 0: ws /= denom
...
def sig01(x: float) -> float:
    try: return float(1.0 / (1.0 + np.exp(-x)))
...
class Strategy:
    def detect_regime(self, df5: pd.DataFrame, df15: pd.DataFrame) -> Tuple[str, bool]:
        if df5.empty or df15.empty: return "Unknown", False
        ...
        atr_now = float(atr.iloc[-1]); atr_avg = float(atr.tail(100).mean()) if len(atr) >= 20 else atr_now
        vol_spike = atr_now > atr_avg * 2.2 if atr_avg > 0 else False
        if adx > 30 and not vol_spike: return "Strong Trending", vol_spike
        if adx > 20 and vol_spike: return "Volatile Trending", vol_spike
        if vol_spike: return "High Volatility", vol_spike
    ...
    def features(self, df5, df15, df1h):
        ...
        macd_o = MACD(df5["close"]); macd = macd_o.macd(); macd_s = macd_o.macd_signal()
        rsi = RSIIndicator(df5["close"], 14).rsi()
        bb = BollingerBands(df5["close"], 20, 2); bb_u = bb.bollinger_hband(); bb_l = bb.bollinger_lband()
        atr = AverageTrueRange(df5["high"], df5["low"], df5["close"], 14).average_true_range()
        ...
        don_hi = df5["high"].rolling(20).max(); don_lo = df5["low"].rolling(20).min()
    ...
    def pair_residual_side(self, sym_df: pd.DataFrame, btc_df: pd.DataFrame) -> Optional[Tuple[str, float]]:
        if sym_df.empty or btc_df.empty: return None
    ...
    def alpha_scores(self, symbol, df5, df15, df1h, btc_df, cs_rank, funding_rate):
        ...
        if pr: side, mag = pr; lp, sp = (mag, 0.0) if side == "long" else (0.0, mag)
        else: lp, sp = 0.0, 0.0
        cs_mag = float(np.clip(abs(cs_rank) / 2.5, 0.0, 1.0)); lxs, sxs = (cs_mag, 0.0) if cs_rank > 0 else (0.0, cs_mag)
        cr_l = 0.0; cr_s = 0.0
        if funding_rate is not None:
            if funding_rate > HIGH_POSITIVE_FUNDING: cr_s = float(np.clip((funding_rate - HIGH_POSITIVE_FUNDING) * 4000, 0.0, 1.0))
            elif funding_rate < HIGH_NEGATIVE_FUNDING: cr_l = float(np.clip((HIGH_NEGATIVE_FUNDING - funding_rate) * 4000, 0.0, 1.0))
        return {"trend": (lt, st), "meanrev": (lmr, smr), "vwap": (lvm, svm), "donchian": (ld, sd), "pair": (lp, sp), "xsmom": (lxs, sxs), "carry": (cr_l, cr_s)}

class ScalpingStrategy:
    def __init__(self): pass
    def analyze(self, symbol: str, df1m: pd.DataFrame) -> Optional[dict]:
        if df1m is None or df1m.empty or len(df1m) < max(50, SCALPING_BB_PERIOD + 2): return None
        closes = df1m["close"].values; highs = df1m["high"].values; lows = df1m["low"].values; volumes = df1m["volume"].values
        rsi = RSIIndicator(df1m["close"], SCALPING_RSI_PERIOD).rsi().iloc[-1]
        bb = BollingerBands(df1m["close"], SCALPING_BB_PERIOD, 2); bb_u = bb.bollinger_hband().iloc[-1]; bb_l = bb.bollinger_lband().iloc[-1]
        price = float(closes[-1]); bb_pos = (price - bb_l) / max(1e-9, (bb_u - bb_l))
        vol_ma = float(pd.Series(volumes).rolling(20).mean().iloc[-1] or 0); volume_spike = (volumes[-1] / vol_ma) if vol_ma > 0 else 1.0
        spread = (highs[-1] - lows[-1]) / max(1e-9, price)
        if (rsi < 25 and bb_pos < 0.2 and volume_spike > 1.5 and spread < 0.001):
            return {"side": "long", "entry": price, "sl": price * (1.0 - SCALPING_STOP_LOSS), "tp": price * (1.0 + SCALPING_PROFIT_TARGET), "confidence": float(min((30 - rsi) / 30 + (0.2 - bb_pos), 1.0)), "time_limit": SCALPING_TIME_LIMIT}
        if (rsi > 75 and bb_pos > 0.8 and volume_spike > 1.5 and spread < 0.001):
            return {"side": "short", "entry": price, "sl": price * (1.0 + SCALPING_STOP_LOSS), "tp": price * (1.0 - SCALPING_PROFIT_TARGET), "confidence": float(min((rsi - 70) / 30 + (bb_pos - 0.8), 1.0)), "time_limit": SCALPING_TIME_LIMIT}
        return None

class MeanReversionStrategy:
    def __init__(self): pass
    def analyze(self, symbol: str, df: pd.DataFrame) -> Optional[dict]:
        if df is None or df.empty or len(df) < max(50, ZSCORE_PERIOD + 2): return None
        closes = df["close"].values.astype(float); series = pd.Series(closes)
        roll_mean = series.rolling(ZSCORE_PERIOD).mean(); roll_std = series.rolling(ZSCORE_PERIOD).std()
        z = (series - roll_mean) / (roll_std + 1e-9); zlast = float(z.iloc[-1]) if not np.isnan(z.iloc[-1]) else 0.0
        price = float(closes[-1]); mean_price = float(np.mean(closes[-ZSCORE_PERIOD:])) if len(closes) >= ZSCORE_PERIOD else price
        if zlast < -ZSCORE_THRESHOLD: return {"side": "long", "entry": price, "sl": price * 0.98, "tp": mean_price, "confidence": float(min(abs(zlast) / 3.0, 1.0))}
        if zlast > ZSCORE_THRESHOLD: return {"side": "short", "entry": price, "sl": price * 1.02, "tp": mean_price, "confidence": float(min(abs(zlast) / 3.0, 1.0))}
        return None

class PortfolioManager:
    def __init__(self, bot: "OmegaXBot"): self.bot = bot
    def exposure(self) -> Dict[str, float]:
        with self.bot.lock:
            items = list(self.bot.positions.items())
        res: Dict[str, float] = {}
        for _, p in items:
            if not p or p.is_hedge: continue
    ...
    def bucket_violation(self) -> Optional[str]:
        ...
        if abs(majors_exp) > 0.65 * bal: return "majors"
        if abs(alts_exp) > 0.55 * bal: return "alts"
        return None
...
class ExecutionEngine:
    def __init__(self, mode: str, ex_wrapper: Optional[ExchangeWrapper], persistence: BasePersistence, notifier: Notifier, lock: RLock):
        self.mode = mode; self.exw = ex_wrapper; self.persistence = persistence; self.notifier = notifier; self.lock = lock
        self.pending_intents: List[dict] = []; self.repair_tasks: Dict[str, dict] = {}
    ...
    def _parse_fill(self, order) -> Tuple[Optional[float], Optional[float], Optional[str]]:
        ...
        if avg is None and "info" in order:
            info = order["info"] or {}; avg = info.get("avgPrice") or info.get("ap")
        if avg is None and order.get("cost") and order.get("filled"):
            try: avg = float(order["cost"]) / float(order["filled"])
    ...
    def _place_brackets(self, symbol, side, qty, sl, tp) -> Tuple[Optional[str], Optional[str]]:
        if self.mode == Mode.PAPER: return "SL-PAPER", "TP-PAPER"
        sl_id = None; tp_id = None
        if sl and sl > 0:
            try:
                res_sl = self.exw.ex.create_order(symbol, type="STOP_MARKET", side=self._opposite(side), amount=qty, params={"stopPrice": sl, "reduceOnly": True})
                _, _, sl_id = self._parse_fill(res_sl)
            except Exception as e:
                logging.warning(f"SL place failed {symbol}: {e}")
        if tp and tp > 0:
            try:
                res_tp = self.exw.ex.create_order(symbol, type="TAKE_PROFIT_MARKET", side=self._opposite(side), amount=qty, params={"stopPrice": tp, "reduceOnly": True})
    ...
    def _cancel_brackets(self, symbol, pos: "Position"):
        if self.mode == Mode.PAPER:
            pos.sl_order_id = None; pos.tp_order_id = None; return
        if pos.sl_order_id:
            try: self.exw.cancel_order_safe(pos.sl_order_id, symbol)
            except Exception: pass
            pos.sl_order_id = None
        if pos.tp_order_id:
            try: self.exw.cancel_order_safe(pos.tp_order_id, symbol)
            except Exception: pass
            pos.tp_order_id = None
    def open(self, intent, positions, risk, notifier, hedge_book=None) -> bool:
        symbol, side, qty, entry, sl, tp, lev, mode, tag, time_limit = intent
        now = time.time()
        if self.mode == Mode.PAPER:
            with self.lock:
                positions[symbol] = Position(symbol, side, qty, entry, sl, tp, lev, now, mode=mode, tag=tag, time_limit=(now + time_limit) if time_limit else None)
            notifier.send(f"{symbol}: 🚀 OPEN {mode}/{tag} {side.upper()} qty={qty:.6f} @ {entry:.4f} SL={sl:.4f} TP={tp:.4f} lev={lev}x")
            self._persist(risk, positions, hedge_book); return True
        try:
            self.exw.set_leverage_safe(symbol, lev)
            client_id = f"ox-{int(time.time()*1000)}-{random.randint(100,999)}"
            with self.lock:
                self.pending_intents.append({"symbol": symbol, "side": side, "qty": qty, "sl": sl, "tp": tp, "lev": lev, "mode": mode, "tag": tag, "clientOrderId": client_id, "ts": now})
            self._persist(risk, positions, hedge_book)
            order = self.exw.ex.create_order(symbol, type="market", side=("buy" if side == "long" else "sell"), amount=qty, params={"newClientOrderId": client_id})
            avg, filled, order_id = self._parse_fill(order); avg = avg or entry; q = filled or qty
            with self.lock:
                positions[symbol] = Position(symbol, side, q, float(avg), sl, tp, lev, now, mode=mode, tag=tag, entry_order_id=order_id, client_order_id=client_id, time_limit=(now + time_limit) if time_limit else None)
            if mode in ("hedge", "temp_hedge"):
                ...
                with self.lock:
                    ...
                    if pos: pos.sl_order_id = sl_id; pos.tp_order_id = tp_id
    ...
    def replace_brackets(self, symbol: str, positions, new_qty: float, new_sl: float, new_tp: float, risk, notifier, hedge_book=None):
        ...
        if not p or p.mode in ("hedge", "temp_hedge"): return
        with self.lock:
            p.sl = new_sl; p.tp = new_tp; self._cancel_brackets(symbol, p)
        if self.mode == Mode.PAPER:
            with self.lock: p.sl_order_id = "SL-PAPER"; p.tp_order_id = "TP-PAPER"
            self._persist(risk, positions, hedge_book); return
        sl_id, tp_id = self._place_brackets(symbol, p.side, new_qty, new_sl, new_tp)
        with self.lock:
            pos2 = positions.get(symbol)
            if not pos2: return
            pos2.sl_order_id = sl_id; pos2.tp_order_id = tp_id
            if not sl_id or not tp_id:
                self.repair_tasks[symbol] = {"side": pos2.side, "qty": new_qty, "sl": new_sl, "tp": new_tp, "attempts": 0, "next_try": time.time() + 3}
        if not sl_id or not tp_id: notifier.send(f"{symbol}: ⚠️ Bracket re-arm incomplete. Repair scheduled.")
        self._persist(risk, positions, hedge_book)
    def partial_close(self, symbol: str, close_qty: float, positions, risk, notifier, hedge_book=None) -> Optional[float]:
        with self.lock:
            p = positions.get(symbol)
        if not p or close_qty <= 0: return None
        if close_qty > p.qty: close_qty = p.qty
        if self.mode == Mode.PAPER: return None
        try:
            res = self.exw.ex.create_order(symbol, type="market", side=self._opposite(p.side), amount=close_qty, params={"reduceOnly": True})
            avg, filled, _ = self._parse_fill(res); fill_qty = filled or close_qty
            t = self.exw.fetch_ticker(symbol); fill_price = avg or t.get("last") or t.get("close")
            pnl_part = p.dir() * fill_qty * (float(fill_price) - p.entry)
            with self.lock:
                pos2 = positions.get(symbol)
                if not pos2: return pnl_part
                pos2.qty -= fill_qty; risk.update_on_close(pnl_part)
            notifier.send(f"{symbol}: ↘️ Partial {fill_qty:.6f} @ {float(fill_price):.4f} pnl=${pnl_part:.2f}")
            with self.lock: still = positions.get(symbol)
            if still and still.qty > 0:
                self.replace_brackets(symbol, positions, still.qty, still.sl, still.tp, risk, notifier, hedge_book)
            else:
                with self.lock: positions[symbol] = None
            return pnl_part
        except Exception as e:
    ...
    def add_size(self, symbol: str, add_qty: float, positions, risk, notifier, hedge_book=None):
        with self.lock: p = positions.get(symbol)
        if not p or add_qty <= 0: return
        if self.mode == Mode.PAPER: return
        try:
            res = self.exw.ex.create_order(symbol, type="market", side=("buy" if p.side == "long" else "sell"), amount=add_qty)
            avg, filled, _ = self._parse_fill(res); fill_qty = filled or add_qty
            t = self.exw.fetch_ticker(symbol); fill_price = avg or t.get("last") or t.get("close")
            with self.lock:
                pos2 = positions.get(symbol)
                if not pos2: return
    ...
    def tick_repair(self, positions, risk, notifier, hedge_book=None, max_attempts: int = 6):
        ...
        for sym, task in tasks:
            if now < task["next_try"]: continue
            with self.lock: p = positions.get(sym)
            if not p:
                with self.lock: self.repair_tasks.pop(sym, None)
                continue
            ...
            with self.lock:
                ...
                if not pos2: self.repair_tasks.pop(sym, None); continue
                if sl_id: pos2.sl_order_id = sl_id
                if tp_id: pos2.tp_order_id = tp_id
                ok = bool(pos2.sl_order_id and pos2.tp_order_id)
                if ok: self.repair_tasks.pop(sym, None)
            if ok:
                notifier.send(f"{sym}: ✅ Brackets repaired."); self._persist(risk, positions, hedge_book); continue
            with self.lock:
                task_ref = self.repair_tasks.get(sym)
                if not task_ref: continue
                task_ref["attempts"] += 1
            ...
            if attempts >= max_attempts:
                ...
                t = self.exw.fetch_ticker(sym); px = t.get("last") or t.get("close")
                if px: self.close(sym, positions, float(px), "NO_BRACKETS_SAFETY", risk, notifier, hedge_book)
    def sync_orders(self, positions):
        try:
            open_orders = self.exw.fetch_open_orders()
        except Exception as e:
            logging.warning(f"open orders sync failed: {e}"); return
        ...
        for sym, pos in items:
            if not pos or sym not in by_sym: continue
            for o in by_sym[sym]:
                typ = str(o.get("type", "")).upper(); side = str(o.get("side", "")).lower()
                info = o.get("info", {}) or {}
    ...
    def close(self, symbol, positions, price, reason, risk, notifier, hedge_book=None):
        ...
        if self.mode == Mode.LIVE:
            try:
                with self.lock:
                    ...
                    if p2: self._cancel_brackets(symbol, p2)
        ...
        with self.lock:
            positions[symbol] = None; risk.update_on_close(pnl)
    ...
    def flatten_all(self, positions, prices: Dict[str, float], risk, notifier, hedge_book=None):
        with self.lock: syms = [s for s, p in positions.items() if p]
        for s in syms:
            px = prices.get(s)
            if not px: continue
            self.close(s, positions, px, "EMERGENCY_FLATTEN", risk, notifier, hedge_book)

# ====================== FUNDING / HEDGE / WS / GRID / ARB ======================

class FundingCache:
    def __init__(self, ex: ExchangeWrapper, symbols: List[str]):
        self.ex = ex; self.symbols = symbols; self.last = 0.0; self.rates = {}; self._idx = 0
    def maybe_update(self):
        now = time.time()
        if now - self.last < FUNDING_TTL: return
        try:
            if hasattr(self.ex.ex, "fetchFundingRates"):
                bulk = self.ex.ex.fetchFundingRates(self.symbols)
                if isinstance(bulk, dict):
                    for sym, fr in bulk.items():
                        r = fr.get("fundingRate") or fr.get("info", {}).get("lastFundingRate")
                        if r is not None: self.rates[sym] = float(r)
                self.last = now; return
        except Exception:
            pass
        if not self.symbols: self.last = now; return
        s = self.symbols[self._idx % len(self.symbols)]; self._idx += 1
        try:
            if hasattr(self.ex.ex, "fetchFundingRate"):
                fr = self.ex.ex.fetchFundingRate(s)
                if fr:
                    r = fr.get("fundingRate") or fr.get("info", {}).get("lastFundingRate")
                    if r is not None: self.rates[s] = float(r)
        except Exception as e:
...
class HedgeEngine:
    def __init__(self, bot: "OmegaXBot"):
        self.bot = bot; self.overlay_qty_target: float = 0.0; self.temp_qty_by_underlier: Dict[str, float] = {}; self.last_rebalance = 0.0
    def _beta(self, sym: str, base: str = "BTC/USDT") -> float:
        df_s = self.bot.data.get(sym, "5m"); df_b = self.bot.data.get(base, "5m")
        if df_s.empty or df_b.empty: return 1.0
        rs = df_s["close"].pct_change().dropna().tail(400); rb = df_b["close"].pct_change().dropna().tail(400)
        n = min(len(rs), len(rb)); if n < 120: return 1.0
        rs, rb = rs.iloc[-n:], rb.iloc[-n:]; cov = np.cov(rs, rb)[0, 1]; varb = np.var(rb)
        return float(np.clip(cov / varb if varb > 0 else 1.0, -2.0, 2.0))
    def compute_overlay_target(self):
        with self.bot.lock: items = list(self.bot.positions.items())
        net = 0.0
        for _, p in items:
            if not p or p.is_hedge or p.mode == "recovery": continue
            price = self.bot.prices.get(p.symbol, self.bot.data.get(p.symbol, "5m")); if not price: continue
            net += self._beta(p.symbol) * p.dir() * p.qty * price
        dd = self.bot.risk.drawdown; ratio = 0.6 + min(0.4, dd * 2.0)
        if self.bot.risk.risk_off: ratio = max(ratio, 1.0)
        pb = self.bot.prices.get("BTC/USDT", self.bot.data.get("BTC/USDT", "5m"))
        if pb: self.overlay_qty_target = (-ratio * net) / pb
    def open_temp_hedge_for(self, underlier_sym: str, main_side: str, main_qty: float, main_entry: float):
        pb = self.bot.prices.get("BTC/USDT", self.bot.data.get("BTC/USDT", "5m")); if not pb: return
        notional = main_qty * main_entry * TEMP_ENTRY_HEDGE_FRAC; qty_b = notional / pb
        delta = -qty_b if main_side == "long" else qty_b
        current = self.temp_qty_by_underlier.get(underlier_sym, 0.0); self.temp_qty_by_underlier[underlier_sym] = current + delta
        self.bot.notifier.send(f"🧩 Temp hedge delta for {underlier_sym}: {self.temp_qty_by_underlier[underlier_sym]:.6f}"); self.rebalance()
    def close_temp_hedge_for(self, underlier_sym: str):
        if underlier_sym in self.temp_qty_by_underlier:
            self.temp_qty_by_underlier[underlier_sym] = 0.0; self.bot.notifier.send(f"🧩 Temp hedge cleared for {underlier_sym}"); self.rebalance()
    ...
    def current_hedge_qty(self) -> float:
        with self.bot.lock: p = self.bot.positions.get("BTC/USDT")
        if p and p.is_hedge: return p.dir() * p.qty
        return 0.0
    def rebalance(self):
        if time.time() - self.last_rebalance < 60: return
        self.compute_overlay_target(); target = self.target_net_qty(); cur = self.current_hedge_qty(); delta = target - cur
        sym = "BTC/USDT"; price_b = self.bot.prices.get(sym, self.bot.data.get(sym, "5m"))
        if price_b is None: self.last_rebalance = time.time(); return
        with self.bot.lock: p = self.bot.positions.get(sym)
        if abs(delta) < 1e-9: self.last_rebalance = time.time(); return
        if not p or not p.is_hedge:
            side = "long" if delta > 0 else "short"
            intent = (sym, side, abs(delta), price_b, 0.0, 0.0, 1.0, "hedge", "overlay", 0)
            ...
            if opened:
                with self.bot.lock:
                    ...
                    if pos: pos.is_hedge = True; pos.mode = "hedge"; pos.tag = "hedge"
            self.last_rebalance = time.time(); self.bot.persistence.save(self.bot.risk, self.bot.positions, pending=self.bot.engine.pending_intents, hedge_book=self.snapshot()); return
        sign_cur = 1 if p.side == "long" else -1; desired_sign = 1 if delta > 0 else -1
        if desired_sign == sign_cur:
            ...
            if abs(delta) <= p.qty:
                ...
                intent2 = (sym, side2, remainder, price_b, 0.0, 0.0, 1.0, "hedge", "overlay", 0)
                ...
                if opened2:
                    with self.bot.lock:
                        ...
                        if pos2: pos2.is_hedge = True; pos2.mode = "hedge"; pos2.tag = "hedge"
        self.last_rebalance = time.time(); self.bot.persistence.save(self.bot.risk, self.bot.positions, pending=self.bot.engine.pending_intents, hedge_book=self.snapshot())
...
class UserStream:
    def __init__(self, api_key: str, api_secret: str, exwrap: ExchangeWrapper, enqueue_event):
        self.api_key = api_key; self.api_secret = api_secret; self.exwrap = exwrap; self.enqueue_event = enqueue_event
        self.twm: Optional[ThreadedWebsocketManager] = None
    def start(self):
        if not BINANCE_WS_AVAILABLE:
            logging.warning("UserStream not started: python-binance not installed."); return
        if not self.api_key or not self.api_secret:
            logging.warning("UserStream not started: API keys missing."); return
        try:
            self.twm = ThreadedWebsocketManager(api_key=self.api_key, api_secret=self.api_secret)
            self.twm.start(); self.twm.start_futures_user_socket(callback=self._handler); logging.info("UserStream started.")
        except Exception as e:
    ...
    def _handler(self, msg: dict):
        try:
            ...
            o = msg.get('o', {})
            market_id = o.get('s'); symbol = self.exwrap.id_to_symbol(market_id) or f"{market_id[:-4]}/{market_id[-4:]}"
            ord_type = o.get('o', ''); status = o.get('X', ''); side = o.get('S', '')
            order_id = str(o.get('i') or o.get('orderId')); last_fill = float(o.get('L') or 0.0)
            avg_price = float(o.get('ap') or last_fill or 0.0)
            reduce_only = str(o.get('R', 'false')).lower() == 'true' or str(o.get('r', 'false')).lower() == 'true'
            ev = OrderEvent(symbol, ord_type, status, side, order_id, avg_price, reduce_only); self.enqueue_event(ev)
        except Exception as e:
            logging.debug(f"UserStream handler error: {e}")

class GridManager:
    def __init__(self, bot: "OmegaXBot"):
        self.bot = bot; self.cooldown: Dict[str, float] = {}
    def _can_fire(self, sym: str) -> bool:
        return time.time() >= self.cooldown.get(sym, 0.0)
    def _arm_cooldown(self, sym: str):
        self.cooldown[sym] = time.time() + GRID_COOLDOWN_SEC
    def evaluate(self, sym: str, df5: pd.DataFrame, price: float):
        with self.bot.lock:
            if self.bot.risk.risk_off: return
            open_dirs = sum(1 for p in self.bot.positions.values() if p and not p.is_hedge and p.mode != "recovery")
            if open_dirs >= MAX_CONCURRENT_POS: return
            if self.bot.positions.get(sym): return
        if df5 is None or df5.empty or len(df5) < 100 or not self._can_fire(sym): return
        if self.bot.portfolio_manager.bucket_violation(): return
        if not self.bot.risk.can_trade(sym): return
        highs = df5["high"].tail(288); lows = df5["low"].tail(288)
        if len(highs) < 50: return
        h, l = float(np.max(highs)), float(np.min(lows)); mid = (h + l) / 2.0; span = max(1e-9, (h - l))
        level_gap = max(price * GRID_SPACING_PCT, span / (GRID_LEVELS * 2.0))
        idx = int((price - mid) / level_gap); up_level = mid + ((idx + 1) * level_gap); dn_level = mid + ((idx - 1) * level_gap)
        tol = level_gap * 0.4; side = None
        if abs(price - dn_level) <= tol: side = "long"
        elif abs(price - up_level) <= tol: side = "short"
        else: return
        ...
        tp = price + 1.4 * atr if side == "long" else price - 1.4 * atr
        alpha_boost = self.bot.alpha.boost(sym); lev = self.bot.risk.dynamic_leverage(False, alpha_boost)
        base = (RISK_PER_TRADE / max(1, len(self.bot.symbols))) * self.bot.risk.risk_multiplier(False, alpha_boost) * GRID_RISK_FRACTION
        with self.bot.lock: open_list = [p for p in self.bot.positions.values() if p]
        qty = self.bot.risk.position_size_qty(price, sl, base, open_list)
        if qty <= 0: return
        intent = (sym, side, qty, price, sl, tp, lev, "grid", "grid", 0)
        opened = self.bot.engine.open(intent, self.bot.positions, self.bot.risk, self.bot.notifier, hedge_book=self.bot.hedger.snapshot())
        if opened:
            self.bot.risk.note_open(sym); self.bot.hedger.open_temp_hedge_for(sym, side, qty, price); self._arm_cooldown(sym)

class ArbitrageCoordinator:
    def __init__(self, notifier: Notifier):
        self.notifier = notifier; self.exchanges: Dict[str, ccxt.Exchange] = {}
        for exid in ARB_EXCHANGES:
            try:
                exclass = getattr(ccxt, exid); ex = exclass({"enableRateLimit": True, "timeout": HTTP_TIMEOUT * 1000})
                ex.load_markets(); self.exchanges[exid] = ex
            except Exception as e:
                logging.warning(f"Arb exchange {exid} init failed: {e}")
        self.last_scan = 0.0
    def scan(self, symbols: List[str]):
        if not ENABLE_ARBITRAGE or not self.exchanges or time.time() - self.last_scan < ARB_SCAN_INTERVAL_SEC: return
        self.last_scan = time.time()
        try:
            subset = symbols[:min(20, len(symbols))]
            for sym in subset:
                quotes = []
                for exid, ex in self.exchanges.items():
                    try:
                        ...
                        if t and t.get("bid") and t.get("ask"):
                            quotes.append((exid, float(t["bid"]), float(t["ask"])))
                    except Exception:
                        continue
                best = None
                for i in range(len(quotes)):
                    for j in range(len(quotes)):
                        if i == j: continue
                        ex_buy, _, ask = quotes[i]; ex_sell, bid, _ = quotes[j]
                        if ask <= 0: continue
                        gross = (bid - ask) / ask; net = gross - 0.002
                        if net > ARB_MIN_NET_PCT: best = (sym, ex_buy, ex_sell, net, bid, ask)
                if best:
                    sym, buy_ex, sell_ex, net, bid, ask = best
                    self.notifier.send(f"🔁 Arbitrage: {sym} buy@{buy_ex} {ask:.4f} -> sell@{sell_ex} {bid:.4f} | net {net*100:.2f}%")
        except Exception as e:
            logging.warning(f"Arbitrage scan error: {e}")
...
class OmegaXBot:
    def __init__(self):
        ...
        self.notifier.send_critical(f"🚀 Starting OmegaX Fusion v24 ({MODE}).")
        self.ex = ExchangeWrapper(); markets = self.ex.ex.load_markets()

        if SYMBOL_AUTO_DISCOVERY: candidates = self.ex.discover_top_usdt_symbols(limit=SYMBOLS_TOP_N)
        else: candidates = [SYMBOL_ALIASES.get(s, s) for s in SEED_SYMBOLS]
        normalized = [s for s in candidates if s in markets]
        for s in candidates:
            if s not in markets: logging.warning(f"Skipping {s}: not found on Binance futures.")
        self.symbols = normalized
        self.scalp_symbols = set(self.symbols[:min(SCALPING_MAX_SYMBOLS, len(self.symbols))])

        self.keepalive = KeepAlive(KEEPALIVE_URL, SELF_URL, KEEPALIVE_INTERVAL_SEC); self.keepalive.start()
        ...
        self.strategy = Strategy(); self.scalper = ScalpingStrategy() if ENABLE_SCALPING else None
        ...
        self.alpha = AlphaEngine(self.symbols); self.meta = MetaPolicy(self.symbols)
        self.persistence = build_persistence()
        self.positions: Dict[str, Optional[Position]] = {s: None for s in self.symbols}
        self.lock = RLock(); self.portfolio_manager = PortfolioManager(self)
        ...
        self.funding = FundingCache(self.ex, self.symbols)
        self.last_report = 0.0; self._last_loop_error_notify = 0.0; self._last_reconcile = 0.0

        self.events: Queue[OrderEvent] = Queue()

        self.user_ws: Optional[UserStream] = None
        if MODE == "live" and USE_USER_WS:
            self.user_ws = UserStream(os.environ.get("BINANCE_API_KEY", ""), os.environ.get("BINANCE_API_SECRET", ""), self.ex, self.enqueue_event)
            self.notifier.send("🟡 Starting user WebSocket...")

        self._start_health_server()
        self._restore_state()
        if MODE == "live":
            self._sync_from_exchange(); self.engine.sync_orders(self.positions)
            if self.user_ws: self.user_ws.start(); self.notifier.send("🟢 User WebSocket started.")
        ...
        def _sig_handler(sig, frame):
            try: self.notifier.send_critical(f"🛑 Received signal {sig}. Shutting down.")
            except Exception: pass
            os._exit(0)
        try:
    ...
    def _drain_events(self):
        while True:
            ...
            if ev.status != "FILLED": continue
            is_bracket = ev.ord_type.upper() in ("STOP", "STOP_MARKET", "TAKE_PROFIT", "TAKE_PROFIT_MARKET") or ev.reduce_only
            if not is_bracket: continue
            fill = ev.avg_price if ev.avg_price > 0 else (self.prices.get(ev.symbol, self.data.get(ev.symbol, "5m")) or 0.0)
            if fill: self.engine.external_filled_close(ev.symbol, float(fill), self.positions, self.risk, self.notifier, reason=f"{ev.ord_type}_FILLED", hedge_book=self.hedger.snapshot())

    def _start_health_server(self):
        app = Flask(__name__)
        @app.route("/", methods=["GET", "HEAD"])
        def index(): return "OmegaX Fusion v24 running. Endpoints: /health, /metrics, /ping, /api/status, /api/positions", 200
        ...
        def _h():
            with self.lock:
                ...
                rb = self.risk.balance; dd = self.risk.drawdown; ro = self.risk.risk_off; pu = self.risk.paused_until
            return jsonify({"ok": True, "equity": rb, "dd": dd, "risk_off": ro, "paused_until": pu, "positions": snap_positions, "loss_bucket": self.risk.loss_bucket, "hedge_book": self.hedger.snapshot()}), 200
        @app.route("/metrics", methods=["GET", "HEAD"])
        def _m():
            with self.lock:
                open_positions = sum(1 for p in self.positions.values() if p); rb = self.risk.balance; dd = self.risk.drawdown; daily = self.risk.daily_pnl
            return jsonify({"equity": rb, "dd": dd, "open_positions": open_positions, "daily_pnl": daily}), 200
        @app.route("/api/status", methods=["GET"])
        def _status():
            try:
                with self.lock:
                    s = {"status": "running", "mode": MODE, "uptime_sec": int(time.time() - self.start_time) if hasattr(self, "start_time") else 0}
            ...
            try:
                with self.lock:
                    ...
                    for p in self.positions.values():
                        if not p: continue
                        res.append({"symbol": p.symbol, "side": p.side, "qty": p.qty, "entry": p.entry, "sl": p.sl, "tp": p.tp, "is_hedge": p.is_hedge, "opened_at": p.opened_at})
                return jsonify(res), 200
            except Exception as e:
                return jsonify({"error": str(e)}), 500
        threading.Thread(target=app.run, kwargs={"host": "0.0.0.0", "port": PORT}, daemon=True).start()
        self.notifier.send("🟢 Health server up. Endpoints: /, /ping, /health, /metrics, /api/status, /api/positions")
        ...
        with self.lock:
            ...
            for s, p in st.get("positions", {}).items():
                if p and s in self.positions:
                    try: self.positions[s] = Position(**p)
                    except Exception: self.positions[s] = None
    ...
    def _sync_from_exchange(self):
        try:
            raw = self.ex.fetch_positions_live(); live_map = {}
            for r in raw:
                market_id = r.get("symbol") or r.get("info", {}).get("symbol"); sym = self.ex.id_to_symbol(market_id) or market_id
                ...
                side = "long" if amt > 0 else "short"; entry = float(r.get("entryPrice") or r.get("entry_price") or 0.0)
                live_map[sym] = Position(sym, side, abs(amt), entry, 0.0, 0.0, 1.0, time.time(), mode="directional", tag="sync")
            if live_map:
                with self.lock:
                    for s in self.symbols: self.positions[s] = live_map.get(s)
    ...
    def _reconcile_positions_live(self):
        if MODE != "live": return
        if time.time() - self._last_reconcile < 30: return
        self._last_reconcile = time.time()
        try:
            live_raw = self.ex.fetch_positions_live(); live_map = {}
            for r in live_raw:
                market_id = r.get("symbol") or r.get("info", {}).get("symbol"); sym = self.ex.id_to_symbol(market_id) or market_id
                if sym not in self.symbols: continue
                amt = float(r.get("positionAmt") or r.get("contracts") or r.get("size") or 0)
    ...
    def _on_signal_event(self, ev: Event):
        sig = ev.data or {}; s = sig.get("symbol"); side = sig.get("side"); sl = sig.get("sl"); tp = sig.get("tp")
        if not s or s not in self.symbols or side not in ("long", "short") or not sl or not tp: return
        price = self.prices.get(s, self.data.get(s, "5m")); if not price: return
        with self.lock: has_pos = bool(self.positions.get(s))
        if has_pos or self.risk.risk_off or self.portfolio_manager.bucket_violation(): return
        f_alpha = self.alpha.boost(s); lev = self.risk.dynamic_leverage(False, f_alpha)
        base = (RISK_PER_TRADE / max(1, len(self.symbols))) * self.risk.risk_multiplier(False, f_alpha)
        with self.lock: open_list = [p for p in self.positions.values() if p]
        qty = self.risk.position_size_qty(price, sl, base, open_list); if qty <= 0: return
        intent = (s, side, qty, price, sl, tp, lev, "directional", "external", 0)
        self.engine.open(intent, self.positions, self.risk, self.notifier, hedge_book=self.hedger.snapshot())

    def _manage_position(self, s: str, price: float, df5: pd.DataFrame):
        with self.lock: p = self.positions.get(s)
        if not p: return
        if p.is_hedge: return
        if p.time_limit and time.time() >= p.time_limit:
            self.engine.close(s, self.positions, price, "SCALP_TIME_LIMIT", self.risk, self.notifier, hedge_book=self.hedger.snapshot()); return
        if (p.side == "long" and price <= p.sl) or (p.side == "short" and price >= p.sl):
            pnl = self._pnl(p, price); self.engine.close(s, self.positions, price, "Stop", self.risk, self.notifier, hedge_book=self.hedger.snapshot())
            self.meta.update(s, p.tag, pnl / max(1e-9, abs(p.entry - p.sl) * max(p.qty, 1.0))); self.alpha.update(s, pnl); return
        if (p.side == "long" and price >= p.tp) or (p.side == "short" and price <= p.tp):
            pnl = self._pnl(p, price); self.engine.close(s, self.positions, price, "Target", self.risk, self.notifier, hedge_book=self.hedger.snapshot())
            self.meta.update(s, p.tag, pnl / max(1e-9, abs(p.entry - p.sl) * max(p.qty, 1.0))); self.alpha.update(s, pnl); return
        r = abs(p.entry - p.sl); atr = AverageTrueRange(df5["high"], df5["low"], df5["close"], 14).average_true_range().iloc[-1]
        changed = False
        if r > 0:
            r_mult = self._pnl(p, price) / (r * max(p.qty, 1e-9))
            if r_mult >= TEMP_HEDGE_CLOSE_AT_R: self.hedger.close_temp_hedge_for(s)
            if r_mult >= PARTIAL_AT_R and not p.partial:
                ...
                if self.engine.mode == Mode.PAPER:
                    ...
                    with self.lock:
                        p2 = self.positions.get(s); if not p2: return
                        p2.qty -= close_qty; p2.partial = True; self.risk.update_on_close(pnl_part)
                    self.notifier.send(f"{s}: Partial ✔️ +${pnl_part:.2f}"); changed = True
                else:
                ...
                with self.lock:
                    ...
                    if p3:
                        if p3.side == "long": p3.sl = max(p3.sl, price - atr * 1.2)
                        else: p3.sl = min(p3.sl, price + atr * 1.2)
                        changed = True
            trail_mult = TRAIL_ATR_BASE
            if r_mult >= 3: trail_mult = TRAIL_ATR_TIGHT3
            elif r_mult >= 2: trail_mult = TRAIL_ATR_TIGHT2
            with self.lock:
                p4 = self.positions.get(s)
                if p4:
                    if p4.side == "long" and price > p4.entry + atr:
                        new_sl = max(p4.sl, price - atr * trail_mult)
                        if new_sl > p4.sl: p4.sl = new_sl; changed = True
                    elif p4.side == "short" and price < p4.entry - atr:
                        new_sl = min(p4.sl, price + atr * trail_mult)
                        if new_sl < p4.sl: p4.sl = new_sl; changed = True
        with self.lock: p_now = self.positions.get(s)
        if p_now and p_now.pyramids < PYRAMID_MAX_STEPS and atr > 0:
            ...
            if (p_now.side == "long" and price > trigger) or (p_now.side == "short" and price < trigger):
                ...
                if total_risk < MAX_TOTAL_RISK:
                    if self.engine.mode == Mode.PAPER:
                        with self.lock:
                            p_now2 = self.positions.get(s); if not p_now2: return
                            p_now2.entry = (p_now2.entry * p_now2.qty + price * add_qty) / (p_now2.qty + add_qty); p_now2.qty += add_qty
                            p_now2.pyramids += 1; p_now2.last_add_price = price
                    else:
                        self.engine.add_size(s, add_qty, self.positions, self.risk, self.notifier, hedge_book=self.hedger.snapshot())
                        with self.lock:
                            p_now3 = self.positions.get(s); if not p_now3: return
                            p_now3.pyramids += 1; p_now3.last_add_price = price
                    changed = True
        if changed:
            with self.lock: p_final = self.positions.get(s)
            if p_final: self.engine.replace_brackets(s, self.positions, p_final.qty, p_final.sl, p_final.tp, self.risk, self.notifier, hedge_book=self.hedger.snapshot())

    def _periodic_report(self):
        if time.time() - self.last_report < TELEGRAM_REPORT_INTERVAL_SEC: return
        with self.lock:
            rb = self.risk.balance; dd = self.risk.drawdown; daily = self.risk.daily_pnl
            npos = sum(1 for p in self.positions.values() if p and not p.is_hedge)
            hedged = any(p for p in self.positions.values() if p and p.is_hedge)
        msg = f"📣 15m Update | Equity ${rb:.2f} | DD {dd*100:.2f}% | Daily ${daily:.2f} | Open {npos} | Hedge {'ON' if hedged else 'OFF'}"
        logging.info(msg); self.notifier.send(msg); self.last_report = time.time()

    def run(self):
        try:
            logging.info("Cold start: loading OHLCV..."); self.data.cold_start(); logging.info("Done."); self.start_time = time.time()
            while True:
                try:
                    self._drain_events()
                    ...
                    if self.arb: self.arb.scan(self.symbols)
                    self._reconcile_positions_live()

                    df1m_map = {s: closed_bars(self.data.get(s, "1m"), "1m") for s in self.scalp_symbols} if ENABLE_SCALPING else {}
                    df5_map = {s: closed_bars(self.data.get(s, "5m"), "5m") for s in self.symbols}
                    df15_map = {s: closed_bars(self.data.get(s, "15m"), "15m") for s in self.symbols}
                    ...
                    for s in self.symbols:
                        df5 = df5_map[s]; df1h = df1h_map[s]
                        if df5.empty or df1h.empty or len(df1h) < 50: continue
                        mom1h = float(df1h["close"].iloc[-1] / df1h["close"].iloc[-50] - 1.0)
                        closes = df5["close"]; n = min(80, len(closes))
                        slope = 0.0
                        if n >= 20:
                            y = closes.iloc[-n:].values; x = np.arange(n)
                    ...
                    for _, s in priorities:
                        df5 = df5_map[s]; df15 = df15_map[s]; df1h = df1h_map[s]
                        if df5.empty or df15.empty or df1h.empty: continue
                        price = self.prices.get(s, df5); if price is None: continue
                        regime, vol_spike = self.strategy.detect_regime(df5, df15)
                        with self.lock: has_pos = bool(self.positions.get(s))
                        if has_pos: self._manage_position(s, price, df5); continue

                        if ENABLE_GRID and self.grid: self.grid.evaluate(s, df5, price)

                        if ENABLE_SCALPING and s in self.scalp_symbols and open_dirs < MAX_CONCURRENT_POS and self.risk.can_trade(s) and not self.risk.risk_off and not self.portfolio_manager.bucket_violation():
                            df1m = df1m_map.get(s, pd.DataFrame())
                            if df1m is not None and not df1m.empty:
                                sc = self.scalper.analyze(s, df1m) if self.scalper else None
                                if sc and sc["confidence"] >= 0.7:
                                    side = sc["side"]; sl = float(sc["sl"]); tp = float(sc["tp"])
                                    lev = self.risk.dynamic_leverage(vol_spike, self.alpha.boost(s))
                                    base = (RISK_PER_TRADE / max(1, len(self.symbols))) * 0.6 * self.risk.risk_multiplier(vol_spike, self.alpha.boost(s))
                                    with self.lock: open_list = [p for p in self.positions.values() if p]
                                    qty = self.risk.position_size_qty(price, sl, base, open_list)
                                    if qty > 0:
                                        intent = (s, side, qty, price, sl, tp, lev, "directional", "scalping", sc.get("time_limit", 0))
                                        opened = self.engine.open(intent, self.positions, self.risk, self.notifier, hedge_book=self.hedger.snapshot())
                                        if opened:
                                            open_dirs += 1; self.risk.note_open(s); self.hedger.open_temp_hedge_for(s, side, qty, price); continue

                        if ENABLE_MEAN_REVERSION and open_dirs < MAX_CONCURRENT_POS and self.risk.can_trade(s) and not self.risk.risk_off and not self.portfolio_manager.bucket_violation():
                            mr = self.meanrev.analyze(s, df5) if self.meanrev else None
                            if mr and mr["confidence"] >= 0.65:
                                side = mr["side"]; sl = float(mr["sl"]); tp = float(mr["tp"])
                                lev = self.risk.dynamic_leverage(vol_spike, self.alpha.boost(s))
                                base = (RISK_PER_TRADE / max(1, len(self.symbols))) * self.risk.risk_multiplier(vol_spike, self.alpha.boost(s))
                                with self.lock: open_list = [p for p in self.positions.values() if p]
                                qty = self.risk.position_size_qty(price, sl, base, open_list)
                                if qty > 0:
                                    intent = (s, side, qty, price, sl, tp, lev, "directional", "meanrev", 0)
                                    opened = self.engine.open(intent, self.positions, self.risk, self.notifier, hedge_book=self.hedger.snapshot())
                                    if opened:
                                        open_dirs += 1; self.risk.note_open(s); self.hedger.open_temp_hedge_for(s, side, qty, price); continue

                        if open_dirs >= MAX_CONCURRENT_POS: continue
                        if not self.risk.can_trade(s): continue
                        if self.portfolio_manager.bucket_violation(): continue
                        if not self.risk.risk_off:
                            ...
                            side = "long" if long_score >= short_score else "short"; agg_val = max(long_score, short_score)
                            thr = HV_SCORE_THRESHOLD if "High Volatility" in regime else BASE_SCORE_THRESHOLD
                            alpha_boost = self.alpha.boost(s); thr *= (1.0 - min(0.2, max(0.0, alpha_boost)))
                            if agg_val < thr: continue
                            f = self.strategy.features(df5, df15, df1h)
                            atr = float(f["atr"].iloc[-1]) if not np.isnan(f["atr"].iloc[-1]) else max(1e-8, float(df5["close"].diff().abs().tail(14).mean()))
                            sl_mult = 1.2 if "Ranging" in regime else 1.6; tp_mult = 1.6 if "Trending" in regime else 1.2
                            sl = price - atr * sl_mult if side == "long" else price + atr * sl_mult
                            tp = price + atr * tp_mult if side == "long" else price - atr * tp_mult
                            if sl <= 0 or tp <= 0 or sl == price or tp == price: continue
                            ...
                            intent = (s, side, qty, price, sl, tp, lev, "directional", tag, 0)
                            opened = self.engine.open(intent, self.positions, self.risk, self.notifier, hedge_book=self.hedger.snapshot())
                            if opened:
                                open_dirs += 1; self.risk.note_open(s); self.hedger.open_temp_hedge_for(s, side, qty, price)

                        if RECOVERY_ENABLED and self._can_open_recovery(open_dirs):
                            f = self.strategy.features(df5, df15, df1h); atr = float(f["atr"].iloc[-1]); slope = self.strategy.slope_dir(df5["close"]); px = float(df5["close"].iloc[-1])
                            mr_long = (px < f["bb_l"].iloc[-1] and f["rsi"].iloc[-1] < 32)
                            mr_short = (px > f["bb_u"].iloc[-1] and f["rsi"].iloc[-1] > 68)
                            vwap_up = (px > f["vwap"].iloc[-1] and f["macd"].iloc[-1] > 0); vwap_down = (px < f["vwap"].iloc[-1] and f["macd"].iloc[-1] < 0)
                            r_side = None
                            if slope == "up" and (mr_long or vwap_up): r_side = "long"
                            if slope == "down" and (mr_short or vwap_down): r_side = "short"
                            if r_side and atr > 0:
                                ...
                                if budget > 0:
                                    ...
                                    if qty_r > 0:
                                        intent = (s, r_side, qty_r, price, sl_r, tp_r, 1.0, "recovery", "recovery", 0)
                                        opened = self.engine.open(intent, self.positions, self.risk, self.notifier, hedge_book=self.hedger.snapshot())
                                        if opened:
                                            open_dirs += 1; self.risk.note_open(s); self.hedger.open_temp_hedge_for(s, r_side, qty_r, price)
            ...
            try: self.notifier.send_critical(f"💥 OmegaX Fusion fatal crash: {str(e)[:200]}")
            except Exception: pass
            raise

    def _can_open_recovery(self, open_dirs: int) -> bool:
        if self.risk.loss_bucket <= 0: return False
        if open_dirs >= MAX_CONCURRENT_POS: return False
        with self.lock: active = sum(1 for p in self.positions.values() if p and p.mode == "recovery")
...
if __name__ == "__main__":
    try:
        bot = OmegaXBot(); bot.run()
    except KeyboardInterrupt:
        try: Notifier(TELEGRAM_ENABLED).send_critical("🛑 OmegaX Fusion stopped by KeyboardInterrupt.")
        except Exception: pass
        raise
    except Exception as e:
        logging.exception("Fatal crash")
        try: Notifier(TELEGRAM_ENABLED).send_critical(f"💥 OmegaX Fusion crashed: {str(e)[:200]}")
        except Exception: pass
        raise