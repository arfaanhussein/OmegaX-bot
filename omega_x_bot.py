...
- Python 3.13.4
- Exchange: Binance USDT-M Futures (ccxt for REST, python-binance for user WebSocket)
...
- Fixes:
  ...
  - Private user WebSocket for order fills -> resolves cancel/replace race (immediate reconciliation)
  - Sync live open orders on startup to restore bracket IDs
  - Overlay hedge and per-trade temp hedges are separate logical entities; reconciled to one net BTC hedge
  - Peak-equity DD + graceful flatten
  - Closed-candle signals; continuous strengths
...
def sleep_jitter(a: float, b: float): time.sleep(random.uniform(a, b))
...
def closed_bars(df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    ...
    return df.iloc[:-1].copy() if now_ms < (last_ts + tf_ms) else df.copy()
...
class Backoff:
    ...
    def fail(self):
        ...
        logging.warning(f"Backoff {d:.2f}s"); time.sleep(d); self.mult *= self.base
    def success(self): self.mult = 1.0
    ...
    def fetch_ohlcv(self, s, tf, limit=300):
        while True:
            try:
                ...
                self.box.success(); return d
            except (ccxt.RateLimitExceeded, ccxt.NetworkError) as e:
                logging.warning(f"fetch_ohlcv {s} {tf}: {e}"); self.box.fail()
            except Exception as e:
                logging.error(f"fetch_ohlcv fatal {s} {tf}: {e}"); self.box.fail()
    def fetch_ticker(self, s):
        while True:
            try:
                d = self.ex.fetch_ticker(s); self.box.success(); return d
            except (ccxt.RateLimitExceeded, ccxt.NetworkError) as e:
                logging.warning(f"fetch_ticker {s}: {e}"); self.box.fail()
            except Exception as e:
                logging.error(f"fetch_ticker fatal {s}: {e}"); self.box.fail()
    def fetch_open_orders(self, symbol: Optional[str] = None):
        try: return self.ex.fetch_open_orders(symbol)
        except Exception: return []
    def cancel_order_safe(self, order_id: str, symbol: str):
        try: return self.ex.cancel_order(order_id, symbol)
        except Exception as e:
...
class DataFeed:
    ...
    def cold_start(self):
        for s in self.syms:
            for tf in self.tfs:
                self._update_one(s, tf); sleep_jitter(0.2, 0.5)
    def _update_one(self, s, tf):
        ...
        self.df[s][tf] = df; self.last[s][tf] = time.time()
    def schedule_updates(self, max_updates=MAX_KLINE_UPDATES_PER_LOOP) -> int:
        ...
        for s in self.syms:
            for tf in self.tfs:
                ...
                if now - due >= -1.0: tasks.append((now-due, s, tf))
        tasks.sort(reverse=True)
        cnt = 0
        for _, s, tf in tasks[:max_updates]:
            try: self._update_one(s, tf); cnt += 1
            except Exception as e: logging.warning(f"update {s} {tf} failed: {e}")
        return cnt
    def get(self, s, tf): return self.df[s][tf]

class Persistence:
    def __init__(self, path: str = STATE_FILE): self.path = path
    def load(self):
        if not os.path.exists(self.path): return {}
        try: 
            with open(self.path,"r",encoding="utf-8") as f: return json.load(f)
        except Exception as e:
            logging.warning(f"State load failed: {e}"); return {}
    def save(self, risk, positions: Dict[str, Optional["Position"]], pending: List[dict] | None = None, hedge_book: dict | None = None):
        try:
            ...
            tmp=self.path+".tmp"
            with open(tmp,"w",encoding="utf-8") as f: json.dump(serial,f,indent=2)
...
class Position:
    ...
    def dir(self): return 1 if self.side == "long" else -1

class Mode:
    PAPER="paper"; LIVE="live"

class HardStopTriggered(Exception): pass
...
class RiskManager:
    ...
    def roll_day_if_needed(self):
        ...
        if now_midnight > self.day_anchor:
            self.day_anchor = now_midnight; self.daily_pnl = 0.0
    def note_open(self, sym: str): self.cooldown[sym] = time.time() + MIN_TRADE_COOLDOWN_SEC
    def can_trade(self, sym: str) -> bool: return time.time() >= self.cooldown.get(sym, 0.0)
    def update_on_close(self, pnl: float):
        before = self.balance
        self.balance += pnl
        self.equity_peak = max(self.equity_peak, self.balance)
        self.daily_pnl += (self.balance - before)
        if pnl < 0: self.consec_losses += 1; self.loss_bucket += -pnl
        else:
            self.consec_losses = 0
            if self.loss_bucket > 0: self.loss_bucket = max(0.0, self.loss_bucket - pnl*0.5)
        self._safety_checks()
    ...
    def position_size_qty(self, entry: float, sl: float, base_risk_frac: float, open_positions: List[Position]) -> float:
        ...
        if total_potential_loss / max(1e-9, self.balance) >= MAX_TOTAL_RISK: return 0.0
        return max(self.balance * base_risk_frac / abs(entry - sl), 0.0)
    def recovery_risk_budget(self) -> float:
        if not RECOVERY_ENABLED or self.loss_bucket <= 0: return 0.0
        eq = self.balance
...
class SymbolAlpha:
    ...
    def expectancy(self) -> float:
        ...
        wr=len(wins)/len(self.pnls); aw=np.mean(wins) if wins else 0.0; al=np.mean(losses) if losses else 0.0
        return wr*aw + (1-wr)*al
    def score(self) -> float:
        return float(np.clip(self.expectancy()/30.0, -0.3, 0.3))

class AlphaEngine:
    def __init__(self, symbols: List[str]): self.stats={s:SymbolAlpha() for s in symbols}
    def update(self, symbol: str, pnl: float): self.stats[symbol].record(pnl)
    def boost(self, symbol: str) -> float: return self.stats[symbol].score()
...
class Welford:
    ...
    def variance(self)->float: return self.m2/(self.n-1) if self.n>1 else 1.0

class MetaPolicy:
    ALPHAS=["trend","meanrev","vwap","donchian","pair","xsmom","carry"]
    def __init__(self, symbols: List[str]): self.stats={s:{a:Welford() for a in self.ALPHAS} for s in symbols}
    def sample_weights(self, symbol: str)->Dict[str,float]:
        ...
        for a,w in self.stats[symbol].items():
            ...
            noise=std/np.sqrt(max(1,w.n)); out[a]=random.gauss(w.mean, max(1e-3,noise))
        vals=np.array(list(out.values())); vals-=np.max(vals); ws=np.exp(vals); ws/=np.sum(ws) if np.sum(ws)>0 else 1.0
        return {a:float(ws[i]) for i,a in enumerate(out.keys())}
    def update(self, symbol: str, alpha_tag: str, r_multiple: float):
        if alpha_tag in self.ALPHAS: self.stats[symbol][alpha_tag].update(r_multiple)

def sig01(x): return float(1.0/(1.0+np.exp(-x)))
...
class Strategy:
    ...
    def features(self, df5, df15, df1h):
        ...
        return dict(ema_f=ema_f,ema_s=ema_s,macd=macd,macd_s=macd_s,rsi=rsi,bb_u=bb_u,bb_l=bb_l,
                    atr=atr,st_k=st_k,vwap=vwap,sma50_1h=sma50_1h,don_hi=don_hi,don_lo=don_lo,ichi=ichi)
    def slope_dir(self, closes: pd.Series)->Optional[str]:
        n=min(80,len(closes)); 
        if n<20: return None
        y=closes.iloc[-n:].values; x=np.arange(n); slope=np.polyfit(x,y,1)[0]
        return "up" if slope>0 else "down"
    def cross_sectional_mom_rank(self, df1h_map):
        rets={}
        for s,df1h in df1h_map.items():
            if df1h.empty or len(df1h)<50: continue
            rets[s]=df1h["close"].iloc[-1]/df1h["close"].iloc[-50]-1.0
        if not rets: return {}
        vals=np.array(list(rets.values())); z=(vals-vals.mean())/(vals.std()+1e-9)
        return {sym:float(z[i]) for i,sym in enumerate(rets.keys())}
    def pair_residual_side(self, sym_df, btc_df)->Optional[Tuple[str,float]]:
        if sym_df.empty or btc_df.empty: return None
        rs=sym_df["close"].pct_change().dropna().tail(300); rb=btc_df["close"].pct_change().dropna().tail(300)
        n=min(len(rs),len(rb)); 
        if n<120: return None
        rs,rb=rs.iloc[-n:],rb.iloc[-n:]; cov=np.cov(rs,rb)[0,1]; varb=np.var(rb)
        beta=cov/varb if varb>0 else 1.0
        spread=rs-beta*rb; z=(spread-spread.mean())/(spread.std()+1e-9); zl=float(z.iloc[-1])
    ...
    def strength_meanrev(self, px,bb_u,bb_l,rsi,st_k,atr):
        band_mid=(bb_u+bb_l)/2.0; half=(bb_u-bb_l)/2.0+1e-9; z=(px-band_mid)/half
        rsi_bias=(50-rsi)/25.0; st_bias=(50-st_k)/25.0
    ...
    def alpha_scores(self, symbol, df5, df15, df1h, btc_df, cs_rank, funding_rate):
        ...
        px=float(df5["close"].iloc[-1]); last=float(df5["close"].iloc[-2]) if len(df5)>=2 else px
        change=(px-last)/last if last else 0.0
        if abs(change)>0.08: return {}
        ...
        return {"trend":(lt,st),"meanrev":(lmr,smr),"vwap":(lvm,svm),"donchian":(ld,sd),
                "pair":(lp,sp),"xsmom":(lxs,sxs),"carry":(cr_l,cr_s)}
...
class Portfolio:
    ...
    def bucket_violation(self)->Optional[str]:
        e=self.exposure(); totals={"majors":0.0,"alts":0.0}
        for b,members in self.BUCKETS.items():
            for s in members: totals[b]+=e.get(s,0.0)
        bal=self.bot.risk.balance
        for b,v in totals.items():
            if abs(v)>self.BUCKET_MAX_FRAC[b]*bal: return b
        return None

class ExecutionEngine:
    def __init__(self, mode: str, ex_wrapper: Optional[ExchangeWrapper], persistence: Persistence, notifier: Notifier):
        self.mode=mode; self.exw=ex_wrapper; self.persistence=persistence; self.notifier=notifier
        self.pending_intents: List[dict]=[]
        self.repair_tasks: Dict[str, dict]={}  # sym->{side,qty,sl,tp,attempts,next_try}
    def _persist(self, risk, positions, hedge_book=None): self.persistence.save(risk,positions,pending=self.pending_intents, hedge_book=hedge_book or {})
    def _parse_fill(self, order)->Tuple[Optional[float],Optional[float],Optional[str]]:
        ...
        if avg is None and "info" in order:
            info=order["info"]; avg=info.get("avgPrice") or info.get("ap")
    ...
    def _opposite(self, side:str)->str: return "sell" if side=="long" else "buy"
    def _place_brackets(self, symbol, side, qty, sl, tp)->Tuple[Optional[str],Optional[str]]:
        ...
        if sl and sl>0:
            ...
            except Exception as e: logging.warning(f"SL place failed {symbol}: {e}")
        if tp and tp>0:
            try:
            ...
            except Exception as e: logging.warning(f"TP place failed {symbol}: {e}")
        return sl_id,tp_id
    ...
    def open(self, intent, positions, risk, notifier, hedge_book=None)->bool:
        ...
        try:
            ...
            positions[symbol]=Position(symbol,side,q,float(avg),sl,tp,lev,now,mode=mode,tag=tag,entry_order_id=order_id,client_order_id=client_id)
            sl_id,tp_id=self._place_brackets(symbol,side,q,sl,tp)
            positions[symbol].sl_order_id=sl_id; positions[symbol].tp_order_id=tp_id
            ...
            logging.error(f"LIVE open failed {symbol}: {e}"); return False
    def replace_brackets(self, symbol:str, positions, new_qty:float, new_sl:float, new_tp:float, risk, notifier, hedge_book=None):
        p=positions.get(symbol); if not p: return
        p.sl=new_sl; p.tp=new_tp
        self._cancel_brackets(symbol,p)
        if self.mode==Mode.PAPER:
            p.sl_order_id="SL-PAPER"; p.tp_order_id="TP-PAPER"; self._persist(risk,positions,hedge_book); return
        sl_id,tp_id=self._place_brackets(symbol,p.side,new_qty,new_sl,new_tp)
        p.sl_order_id=sl_id; p.tp_order_id=tp_id
        if not sl_id or not tp_id:
    ...
    def partial_close(self, symbol:str, close_qty:float, positions, risk, notifier, hedge_book=None)->Optional[float]:
        p=positions.get(symbol); if not p or close_qty<=0: return None
        ...
        try:
            ...
            p.qty-=fill_qty; risk.update_on_close(pnl_part)
            ...
            logging.error(f"Partial close LIVE failed {symbol}: {e}"); return None
    def add_size(self, symbol:str, add_qty:float, positions, risk, notifier, hedge_book=None):
        p=positions.get(symbol); if not p or add_qty<=0: return
        ...
        try:
            ...
            t=self.exw.fetch_ticker(symbol); fill_price=avg or t.get("last") or t.get("close")
            new_qty=p.qty+fill_qty; p.entry=(p.entry*p.qty+float(fill_price)*fill_qty)/max(1e-9,new_qty); p.qty=new_qty
    ...
    def tick_repair(self, positions, risk, notifier, hedge_book=None, max_attempts:int=6):
        ...
        for sym,task in list(self.repair_tasks.items()):
            ...
            p=positions.get(sym)
            if not p:
                self.repair_tasks.pop(sym,None); continue
            sl_id,tp_id=self._place_brackets(sym,task["side"],task["qty"],task["sl"],task["tp"])
            if sl_id: p.sl_order_id=sl_id
            if tp_id: p.tp_order_id=tp_id
            if p.sl_order_id and p.tp_order_id:
                notifier.send(f"{sym}: ✅ Brackets repaired."); self.repair_tasks.pop(sym,None); self._persist(risk,positions,hedge_book); continue
            task["attempts"]+=1; task["next_try"]=now+min(10,2+task["attempts"]*2)
            if task["attempts"]>=max_attempts:
                ...
                self.repair_tasks.pop(sym,None); self._persist(risk,positions,hedge_book)
    def sync_orders(self, positions):
        try: open_orders=self.exw.fetch_open_orders()
        except Exception as e:
            logging.warning(f"open orders sync failed: {e}"); return
        by_sym: Dict[str,List[dict]]={}
        for o in open_orders: by_sym.setdefault(o.get("symbol",""),[]).append(o)
        for sym,pos in positions.items():
            if not pos or sym not in by_sym: continue
            for o in by_sym[sym]:
                typ=str(o.get("type","")).upper(); side=str(o.get("side","")).lower(); info=o.get("info",{}) or {}
                ro=o.get("reduceOnly"); 
                if ro is None: ro=str(info.get("reduceOnly","false")).lower()=="true"
                if not ro: continue
                sp=o.get("stopPrice") or info.get("stopPrice") or o.get("price")
                ...
                if typ in ("STOP","STOP_MARKET") and sp: pos.sl_order_id=o.get("id") or info.get("orderId"); pos.sl=pos.sl or sp
                if typ in ("TAKE_PROFIT","TAKE_PROFIT_MARKET") and sp: pos.tp_order_id=o.get("id") or info.get("orderId"); pos.tp=pos.tp or sp
    def close(self, symbol, positions, price, reason, risk, notifier, hedge_book=None):
        ...
        if self.mode==Mode.LIVE:
            ...
            except Exception as e: logging.error(f"LIVE close failed {symbol}: {e}")
...
class FundingCache:
    def __init__(self, ex: ExchangeWrapper): self.ex=ex; self.last=0.0; self.rates={}; self._idx=0
    def maybe_update(self):
        ...
        s=SYMBOLS[self._idx]; self._idx=(self._idx+1)%len(SYMBOLS)
        try:
        ...
        except Exception as e: logging.debug(f"funding fetch fail {s}: {e}")
        self.last=now
...
class HedgeEngine:
    ...
    Two logical books:
      ...
      - temp_qty_by_underlier: dict underlier->qty (+long/-short)
    ...
    def _beta(self, sym:str, base:str="BTC/USDT")->float:
        ds=self.bot.data.get(sym,"5m"); db=self.bot.data.get(base,"5m")
        if ds.empty or db.empty: return 1.0
        rs=ds["close"].pct_change().dropna().tail(400); rb=db["close"].pct_change().dropna().tail(400)
        n=min(len(rs),len(rb)); 
        if n<120: return 1.0
        rs,rb=rs.iloc[-n:],rb.iloc[-n:]; cov=np.cov(rs,rb)[0,1]; varb=np.var(rb)
        return float(np.clip(cov/varb if varb>0 else 1.0, -2.0, 2.0))
    def compute_overlay_target(self):
        # net BTC-equivalent exposure
        net=0.0
        for p in self.bot.positions.values():
        ...
        sym="BTC/USDT"; pb=self.bot.prices.get(sym, self.bot.data.get(sym,"5m"))
        if not pb: return
    ...
    def open_temp_hedge_for(self, underlier_sym:str, main_side:str, main_qty:float, main_entry:float):
        ...
        qty_b=notional/pb
        # temp qty sign: + long BTC, - short BTC
        self.temp_qty_by_underlier[underlier_sym]=self.temp_qty_by_underlier.get(underlier_sym,0.0) + (qty_b if main_side=="short" else -qty_b)
    ...
    def close_temp_hedge_for(self, underlier_sym:str):
        if underlier_sym in self.temp_qty_by_underlier:
            ...
            self.bot.notifier.send(f"🧩 Temp hedge closed marker for {underlier_sym}")
            self.rebalance()
    ...
    def current_hedge_qty(self)->float:
        ...
        return p.dir()*p.qty if p and p.is_hedge else 0.0
    def rebalance(self):
        ...
        delta=target - cur
        if abs(delta) < 1e-6:
            self.last_rebalance=time.time(); return
        sym="BTC/USDT"
        # Open/adjust physical hedge
        if delta>0:
            # need to be more long BTC
            if sym not in self.bot.positions or not self.bot.positions[sym]:
                intent=(sym,"long",abs(delta), self.bot.prices.get(sym, self.bot.data.get(sym,"5m")), 0.0,0.0,1.0,"hedge","overlay")
                if self.bot.engine.open(intent,self.bot.positions,self.bot.risk,self.bot.notifier, hedge_book=self.snapshot()):
            ...
            # reduce hedge or go short
            p=self.bot.positions.get(sym)
            if p and p.is_hedge and p.side=="long" and abs(delta)<=p.qty:
                self.bot.engine.partial_close(sym, abs(delta), self.bot.positions, self.bot.risk, self.bot.notifier, hedge_book=self.snapshot())
            else:
                # either no hedge yet or need to flip/add short
                if sym not in self.bot.positions or not self.bot.positions[sym]:
                    intent=(sym,"short",abs(delta), self.bot.prices.get(sym, self.bot.data.get(sym,"5m")), 0.0,0.0,1.0,"hedge","overlay")
                    if self.bot.engine.open(intent,self.bot.positions,self.bot.risk,self.bot.notifier, hedge_book=self.snapshot()):
                        pos=self.bot.positions.get(sym); 
                        if pos: pos.is_hedge=True; pos.mode="hedge"; pos.tag="hedge"
                else:
    ...
    def _handler(self, msg: dict):
        try:
            ...
            if e == 'ORDER_TRADE_UPDATE':
                ...
                last_fill=float(o.get('L') or 0.0)  # last fill px
...
class OmegaXBot:
    def __init__(self):
        ...
        if MODE=="live":
            ...
            if self.user_ws: self.user_ws.start()
    ...
    def _start_health_server(self):
        ...
        def _h():
            return jsonify({
                "ok":True,"equity":self.risk.balance,"dd":self.risk.drawdown,
                "risk_off":self.risk.risk_off,"paused_until":self.risk.paused_until,
        ...
        @app.route("/metrics")
        def _m():
            return jsonify({"equity":self.risk.balance,"dd":self.risk.drawdown,"open_positions":sum(1 for p in self.positions.values() if p),"daily_pnl":self.risk.daily_pnl})
    ...
    def _on_order_update(self, symbol:str, ord_type:str, status:str, side:str, order_id:str, avg_price:float, reduce_only:bool, raw:dict):
        ...
        # Only care about bracket fills here
        if status != "FILLED": return
        p=self.positions.get(symbol)
        if not p: return
        # If this filled order matches known SL/TP or is reduceOnly stop/take-profit
        typ=ord_type.upper()
        is_bracket = typ in ("STOP","STOP_MARKET","TAKE_PROFIT","TAKE_PROFIT_MARKET") or reduce_only
        if not is_bracket: return
        # If it's our bracket id, or we suspect the position is closed, finalize
        if (p.sl_order_id and str(p.sl_order_id)==order_id) or (p.tp_order_id and str(p.tp_order_id)==order_id) or is_bracket:
            # external close: do not send more orders
    ...
    def run(self):
        logging.info("Cold start: loading OHLCV..."); self.data.cold_start(); logging.info("Done.")
        while True:
            try:
                # repair brackets
                ...
                for s in self.symbols:
                    ...
                    closes=df5["close"]; n=min(80,len(closes)); slope=0.0
                    if n>=20:
                        y=closes.iloc[-n:].values; x=np.arange(n); slope=float(np.polyfit(x,y,1)[0]/(np.std(y)+1e-9))
                    priorities.append((mom1h+slope,s))
                priorities.sort(reverse=True)

                for _, s in priorities:
                    df5=df5_map[s]; df15=closed_bars(self.data.get(s,"15m"),"15m"); df1h=df1h_map[s]
                    ...
                    # manage existing
                    if self.positions[s]:
                        self._manage_position(s, price, df5); continue

                    if open_dirs>=MAX_CONCURRENT_POS: continue
                    if not self.risk.can_trade(s): continue
                    if self.portfolio.bucket_violation(): continue
                    ...
                    if not self.risk.risk_off:
                        ...
                        if self.engine.open(intent,self.positions,self.risk,self.notifier, hedge_book=self.hedger.snapshot()):
                            open_dirs+=1; self.risk.note_open(s)
                            # temp hedge (separate logical)
                    ...
                    if RECOVERY_ENABLED and self._can_open_recovery(open_dirs):
                        f=self.strategy.features(df5,df15,df1h)
                        atr=float(f["atr"].iloc[-1]); slope=self.strategy.slope_dir(df5["close"])
                        px=float(df5["close"].iloc[-1]); mr_long=(px<f["bb_l"].iloc[-1] and f["rsi"].iloc[-1]<32)
                ...
                self.persistence.save(self.risk,self.positions, pending=self.engine.pending_intents, hedge_book=self.hedger.snapshot())
                self.risk.paused_until=time.time()+3600; break
            except Exception as e:
                logging.exception(f"Loop error: {e}"); sleep_jitter(1.0, 2.0)
    ...
    def _pnl(self, p:Position, price:float)->float: return p.dir()*p.qty*(price - p.entry)

    def _manage_position(self, s:str, price:float, df5:pd.DataFrame):
        p=self.positions[s]; if not p: return
        # backup client-side checks (native brackets do the real work)
        ...
        if r>0:
            ...
            if r_mult>=PARTIAL_AT_R and not p.partial:
                ...
                if self.engine.mode==Mode.PAPER:
                    pnl_part=p.dir()*close_qty*(price-p.entry); p.qty-=close_qty; p.partial=True; self.risk.update_on_close(pnl_part); self.notifier.send(f"{s}: Partial ✔️ +${pnl_part:.2f}")
                    changed=True
                else:
                    ...
                    if pnl_part is not None:
                        ...
                        if not p: return
                        p.partial=True; changed=True
                if p:
                    if p.side=="long": p.sl=max(p.sl, price - atr*1.2)
                    else: p.sl=min(p.sl, price + atr*1.2)
                    changed=True
            ...
            if p.side=="long" and price>p.entry+atr:
                ...
                if new_sl>p.sl: p.sl=new_sl; changed=True
        ...
        if p and p.pyramids<PYRAMID_MAX_STEPS and atr>0:
            ...
            if (p.side=="long" and price>trigger) or (p.side=="short" and price<trigger):
                ...
                if total_risk<MAX_TOTAL_RISK:
                    if self.engine.mode==Mode.PAPER:
                        p.entry=(p.entry*p.qty+price*add_qty)/(p.qty+add_qty); p.qty+=add_qty; p.pyramids+=1; p.last_add_price=price
                    else:
                        self.engine.add_size(s,add_qty,self.positions,self.risk,self.notifier, hedge_book=self.hedger.snapshot())
                        p=self.positions.get(s); 
                        if not p: return
                        p.pyramids+=1; p.last_add_price=price
                    changed=True
    ...
    def _hourly_report(self):
        ...
        msg=(f"📊 Hourly | Equity ${self.risk.balance:.2f} (Δ ${self.risk.balance-INITIAL_BALANCE:.2f}) "
